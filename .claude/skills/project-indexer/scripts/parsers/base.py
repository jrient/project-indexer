...
@dataclass
class ExportItem:
    """Represents a single exported item with its signature and docstring."""
    signature: str
...
class FileSignature:
    ...
    imports: list[str]
    module_doc: Optional[str] = None  # 模块级 docstring
    internal_deps: list[str] = None  # 项目内部依赖

    def __post_init__(self):
        if self.internal_deps is None:
            self.internal_deps = []
...
class BaseParser(ABC):
    ...
    def format_markdown(self, signature: FileSignature, high_density: bool = False) -> str:
        """
        Format the signature as markdown.

        Args:
            signature: The parsed file signature
            high_density: If True, use compressed format to reduce tokens
        ...
        if signature.module_doc and not high_density:
            lines.append(f"> {signature.module_doc}")
            lines.append("")
        ...
        if signature.exports:
            ...
            for export in signature.exports:
                # 高密度模式：签名 + docstring 在同一行
                if high_density and export.docstring:
                    lines.append(f"- `{export.signature}` - {export.docstring}")
                else:
                    lines.append(f"- `{export.signature}`")
        else:
        ...
        if signature.imports:
            lines.append("")
            if high_density:
                lines.append("**Deps**: " + ", ".join(signature.imports[:8]))
            else:
        ...
        if signature.internal_deps and not high_density:
            lines.append("")
            lines.append("**Internal**: " + ", ".join(f"`{dep}`" for dep in signature.internal_deps[:5]))
